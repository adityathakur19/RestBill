...
const NewEstimate = () => {
  ...
  const validateEstimate = (estimateData) => {
    if (!estimateData.party?._id) {
      throw new Error('Please select a party');
    }
    if (!estimateData.items?.length) {
      throw new Error('Please add at least one item');
    }
    if (!estimateData.billDate) {
  ...
  const handleSaveEstimate = async () => {
    try {
        ...
        const estimateData = {
            ...
            estimateNote: formData.estimateNote,
            transport: transportDetails
        };

        // Optional: Validate estimate data before making the API call
        ...
        if (!response.ok) {
            ...
            if (contentType && contentType.includes('application/json')) {
                ...
                throw new Error(errorData.message || 'Failed to save estimate');
            } else {
        ...
        setError(error.message || 'Error saving estimate');
    } finally {
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <textarea
                value={formData.estimateNote}
                onChange={(e) => setFormData(prev => ({
                  ...prev,
                  estimateNote: e.target.value
                }))}
                className="w-full p-2 border rounded h-24"
